...
discovery loop. The probe is opt-in via `check_llms_txt=True`;
by default the URL is fetched as-is without checking for an `llms.txt` site map.
...
| `check_llms_txt` | bool | `False` | When `True`, probe for an `llms.txt` site map before fetching the URL |
